...
within a dependently typed language. Let's contrast
...
datatype.
For example, we could write a generic \AgdaFun{nodes} function, that
can be applied to \textit{any} datatype.
The \AgdaFun{nodes} function returns the sum of:
\begin{itemize}
\item The number of non-inductive nodes.
\item The \textit{recursive} sum of the number of inductive nodes.
\end{itemize}
Applying \AgdaFun{nodes}
to a list of pairs of booleans
results in the sum of the number of \AgdaCon{cons} constructors,
the \AgdaCon{nil} constructor,
...
For example, we could write a generic \AgdaFun{count} function that
can also be applied to \textit{any} datatype.
The \AgdaFun{count} function returns the sum of:
\begin{itemize}
\item The \textit{recursive} sum of the number of non-inductive nodes.
\item The \textit{recursive} sum of the number of inductive nodes.
\end{itemize}
Applying \AgdaFun{count}
to a list of pairs of booleans
results in the sum of the number of \AgdaCon{cons} constructors,
the \AgdaCon{nil} constructor, the number of pairs
(\AgdaCon{,}), and the number of booleans.
Notably, \AgdaFun{count} (unlike \AgdaFun{nodes}) additionally
recurses through the components of the pair.

In the remainder of this introduction we
provide an overview of dependent types (\refsec{TODO}),
give an example of fully generic programming over a limited collection
of datatypes (\refsec{TODO}),
describe the class of datatypes that we have been able to extend
fully generic programming to (\refsec{TODO}),
